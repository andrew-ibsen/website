...
function animateCounters() {
  ...
  counters.forEach(counter => {
    ...
    let prefix = '';
    let suffix = '';
    // Determine prefix/suffix based on initial text
...
function initMapMarkers() {
  ...
  const locations = [
    {
      name: 'New York (JFK)',
      description: 'Duty Maintenance Manager at British Airways Engineering JFK; manages largest overseas line station generating $3.5M annually.',
      x: 0.27,
      y: 0.38
    },
    {
      name: 'London (Heathrow)',
      description: 'Performance Recovery Team operations at LHR; led time-critical recovery operations.',
      x: 0.45,
      y: 0.30
    },
    {
      name: 'Reykjavik (Iceland)',
      description: 'Licensed Aircraft Engineer for Icelandair; managed line and base maintenance.',
      x: 0.42,
      y: 0.25
    },
    {
      name: 'Geneva (Switzerland)',
      description: 'Future relocation; expanding global operations perspective.',
      x: 0.47,
      y: 0.33
    }
  ];
...
function drawBarChart(canvasId, labels, values, colors) {
  ...
  values.forEach((val, i) => {
    ...
    const y = height - barHeight - 25; // leave space for labels
    ctx.fillStyle = colors[i % colors.length];
    ctx.fillRect(x, y, barWidth, barHeight);
    // Draw value label
    ctx.fillStyle = '#333';
    const valueLabel = val + 'M';
    ctx.fillText(valueLabel, x + barWidth/2 - ctx.measureText(valueLabel).width/2, y - 5);
    // Draw x-axis label
